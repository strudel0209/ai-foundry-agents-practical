...
to search through indexed Employee data and answer user questions about Employees.

Prerequisites:
1. Set AZURE_AI_PROJECT_ENDPOINT and AZURE_AI_MODEL_DEPLOYMENT_NAME environment variables
2. Ensure you have an Azure AI Search connection configured in your Azure AI project
3. The search index "gptkbindex" should exist in your Azure AI Search service
   (you can create this using the Azure portal with sample Employee data)
...
async def main() -> None:
    ...
    async with (
        ...
        azure_ai_agent = await agents_client.create_agent(
            ...
            instructions=(
                "You are a helpful agent that searches Employee information using Azure AI Search. "
                "Always use the search tool and index to find Employee data and provide accurate information."
            ),
        ...
        try:
            async with ChatAgent(
                ...
                instructions=("You are a helpful agent that uses the search tool and index to find useful information for the employees.If the question is not in English, answer in the language used in the question. Each source has a name followed by colon and the actual information, always include the source name for each fact you use in the response. Use square brackets to reference the source, for example [info1.txt]. Don't combine sources, list each source separately, for example [info1.txt][info2.pdf]. {% if image_sources %}"),
            ) as agent:
                print("This agent uses raw Azure AI Search tool to search Employee data.\n")

                # 3. Simulate conversation with the agent
                user_input = (
                    "Use Azure AI search knowledge tool to find detailed information about a employees runbooks."
