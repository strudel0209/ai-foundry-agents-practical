...
async def main(scenario: Literal["chat_client", "chat_client_stream", "ai_function", "all"] = "all"):
    ...
    with get_tracer().start_as_current_span("Sample Scenario's", kind=trace.SpanKind.CLIENT) as current_span:
        ...
        model = os.getenv("OPENAI_CHAT_MODEL_ID") or os.getenv("AZURE_OPENAI_CHAT_DEPLOYMENT_NAME")
        base_url = os.getenv("OPENAI_BASE_URL") or os.getenv("AZURE_OPENAI_ENDPOINT")
        
        # If using Azure key and no explicit base url, try to use Azure endpoint
    ...
    # CRITICAL: Flush telemetry before exiting.
    # In short-lived scripts, the process often exits before the background batch processor sends data.
